(function () {
  ...
  function potHash( idx, hash, X, Y )
  {
    ...
    if( hash[XY] != null ) {
      console.log( XY );
      console.log( Object.keys( hash[XY].eff ).join(',') );
    }
  ...
  function buildBigPotions(idx)
  {
    ...
    for( var i=0; i<max-1; i++ ){
      ...
      for( var j=i+1; j<max; j++ ){
        ...
        {
          var effs = [];
          for( var e=0; e<A.eff.length; e++ ) effs.push( A.eff[e] );
          for( var e=0; e<B.eff.length; e++ ) effs.push( B.eff[e] );

          var pot = {
            n: idx.pot.num++,
            ing: jQuery.unique( [ A.ing[0], A.ing[1], B.ing[0], B.ing[1] ] ),
            eff: jQuery.unique( effs )
          };
